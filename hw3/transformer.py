...
class PositionEmbedding(nn.Module):
    ...
    def forward(self, inputs, start=1):
        ...
        position_shape = (1, sequence_length, -1) # fill in the other two dimensions
...
class TransformerEncoderBlock(nn.Module):
    ...
    def forward(self, inputs, self_attention_mask=None):
        ...
        norm_inputs = self.norm.forward(inputs)

        # Apply the self-attention with the normalized input, use the self_attention mask as the optional mask parameter.
        attn = self.self_attention.forward(inputs=(norm_inputs, norm_inputs), mask=self_attention_mask)
        ...
        output = self.feed_forward.forward(res_attn)
        return output
...
class TransformerDecoderBlock(nn.Module):
    ...
    def forward(self, decoder_inputs, encoder_outputs, self_attention_mask=None, cross_attention_mask=None):    
        ...
        norm_decoder_inputs = self.self_norm.forward(decoder_inputs)

        target_selfattn = self.self_attention.forward((norm_decoder_inputs, norm_decoder_inputs), mask=self_attention_mask)
        ...
        encdec_attention = self.cross_attention.forward((norm_target_selfattn, norm_encoder_outputs), mask=cross_attention_mask)
